%% SHADOWING IN CONVEX QUADRATICS (HB)
...
for k = 2:n_ODE
    if k==2
        ...
        m = x_ODE(:,k-1)-x_ODE(:,k-2);
    end
    x_ODE(:,k) = x_ODE(:,k-1) + (1-eta_ODE*alpha)*m - eta_ODE^2*H*x_ODE(:,k-1);
end
...
for k = 2:n
    ...
    x(:,k) = x(:,k-1) + (1-eta*alpha)*m - eta^2*H*x(:,k-1);
end
...
if d==2
    ...
    l2=legend([h1,h2,h5],{'Heavy Ball: $z_k$','HB-ODE: $ \ \ \ q_k$','solution'},'Location','NorthEast');
    l2.set('FontSize',18,'Interpreter','Latex');
    text(x0(1)-1.1,x0(2)+0.5,'$\ z_0 = q_0$','FontSize',18,'Interpreter','Latex')
    xlim([XX(1),XX(end)]);
    ylim([YY(1),YY(end)]);
    axis equal
    ...
    text(1,predicted+0.17,'GD bound: $\epsilon = \frac{h\ell L}{2\mu}$','FontSize',15,'Interpreter','Latex')
